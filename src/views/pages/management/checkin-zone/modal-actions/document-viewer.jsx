...
const DocumentViewer = ({ isOpen, onClose, bookingId }) => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (res.data?.succeeded) {
          ...
          toast.error(res.data?.message || 'Không thể tải giấy tờ');
        }
      } catch (err) {
        console.error(err);
      ...
      <div className="relative p-4 w-full max-w-lg max-h-full">
        <div className="relative bg-white rounded-lg shadow-sm overflow-auto">
          <div className="flex items-center justify-between p-4 border-b">
            <h3 className="text-lg font-semibold">Xem giấy tờ ({items.length})</h3>
